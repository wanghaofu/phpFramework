...
class StraMatch 
{
	 ...
	 * @var obj
	 */
	private $_stra_udb = NULL;
	
	/**
	 * 初始化
	 */
	public function __construct( $uuid, $session )
	{
		$this->uuid = $uuid;
	...
	public function forceMatch()
	{
		...
		/*******************************************************************/
		/* 配对 Start */
		$force_serial_num = $this->_teamAdd( $uuids );
		foreach ($res as $rs) {
			$uLevel += $rs['level'];
		}
		$ave_level = $uLevel / 3;
	...
	public function straMatch()
	{
		...
		/*******************************************************************/
		/* 配对 Start */
		$stra_serial_num = $this->_teamAdd( $forceIds );//生成地图系列号
	 ...
	 * @return $lastId
	 */
	private function _teamAdd( $array )
	{
		$i = 1;
